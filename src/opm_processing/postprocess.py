...
from opm_processing.imageprocessing.utils import max_z_projection, flatfield_correction
...
def postprocess(
    ...
    if flatfield_after_deskew and max_projection:
        ...
        for chan_idx in range(datastore.shape[2]):
            temp_images = np.squeeze(datastore[:,:,sample_indices,:]).result()
        ...
        for pos_idx in tqdm(range(datastore.shape[1]),desc="pos"):
            for chan_idx in tqdm(range(datastore.shape[2]),desc="chan"):
                corrected_data = flatfield_correction(datastore[:, pos_idx, chan_idx, :], flatfields[chan_idx, :])
            ...
            for pos_idx in range(datastore.shape[1]):
                for chan_idx in range(datastore.shape[2]):
                    layer = viewer.add_image(
                        ...
                        scale=[1,pixel_size_um,pixel_size_um],
