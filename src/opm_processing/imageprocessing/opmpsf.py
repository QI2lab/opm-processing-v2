#!/usr/bin/env python
"""Generate theoretical OPM point-spread functions.
...
def get_skewed_coords(sizes, dc, ds, theta, scan_direction="lateral"):
    ...
    if scan_direction == "lateral":
        ...
        y = (
            ds * np.arange(nimgs)[:, None, None]
            + dc * np.cos(theta) * np.arange(ny_cam)[None, :, None]
        )
        ...
        z = (
            ds * np.arange(nimgs)[:, None, None]
            + dc * np.sin(theta) * np.arange(ny_cam)[None, :, None]
        )
    else:
        raise ValueError(
...
def generate_skewed_psf(
    ...
    roi_skewed_size = get_skewed_roi_size(
        [z_res_mod * 12, xy_res * 12, xy_res * 12], theta, dc, dstage, ensure_odd=True
    )
    # make square
